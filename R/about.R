...
#' about(OM.GD.7)
#' @export
about = function(obj, xlim = c(-134.5,-125), ylim = c(48, 55.5), xfac=1.7, col="red", plot.fig=T, ret.dat=F, p = c(0.05, 0.95)){
  ...
  if(plot.fig){
    ...
    map(xlim = xlim, ylim = ylim, add=T)
    poly = OM@Misc$mapinfo$poly
    if(class(poly)=="sfc_multipolygon")plot(poly, add=T, col=col)
    if(class(poly)=="data.frame")polygon(poly[,1],poly[,2],col=col,border=col)
  ...
  if(plot.fig){
    ...
    if(!is.na(catchy[1])){
      ...
      plot(yrs,catchy,pch=19,col=col,ylab="",xlab="",ylim=c(0,max(catchy)*1.025));grid(); lines(yrs,catchy,col=col)
    ...
    if(!is.na(Fd[1])){
      ...
      matplot(yrs,t(Fq),col="NA",xlab="",ylab="",ylim=c(0,max(Fq)*1.025));grid()
