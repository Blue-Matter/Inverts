...
getStat = function(Hist){
  ...
  outlist = list(SSB_SSB0 = SSB[,nyears]/SSB0,
                 SSB_SSBMSY = SSB[,nyears]/SSBMSY,
                 F_F01 = FM[,nyears]/F01[,nyears],
                 F_FMSY = FM[,nyears]/FMSY)
}
...
#' Status(Hist)
#' @export
Status = function(Hist, plot=T, col="cadetblue"){
...
DFO_status=function(Hist, legpos='left',legpos2="bottomleft"){
  ...
  outlist = getStat(Hist)
  par(mfrow = c(1, 1), mai = c(0.7, 0.6, 0.02,    0.02))
  ...
  mtext("SB/SBMSY", 1, line = 2.2, font = 2)
  mtext("F/FMSY", 2, line = 2.7,  font = 2)
...
#' @export
DFO_status_comp=function(Histlist,p=c(0.05,0.25,0.5,0.75,0.95),abbrev_no=8){
  ...
  miny=2
  locs = miny+1:nc-0.5
  plot(c(0,max(qs)*1.025),c(miny,max(locs)*1.3),col='white',axes=F,xlab="",ylab="",yaxs ="i",xaxs ="i")
  textpos=max(locs)*1.2
  MSEtool:::add_zones(textpos);grid()
  nq = length(p)
  lwds=c(2,6,10)
  for(i in 1:nc){
    for(qq in 1:((nq-1)/2))lines(c(qs[i,qq],qs[i,nq-qq+1]),rep(locs[i],2),lwd=lwds[qq])
    lines(rep(qs[i,ceiling(nq/2)],2),locs[i]+c(-0.03,0.03),lwd=4)
  }
  axis(1,c(-1E10,1E10),rep("",2)); axis(1)
  axis(2,c(-1E10,1E10),rep("",2)); axis(2,locs,nams,cex=0.7)
  mtext("SSB/SSBMSY",1,line=2.2)
